...
def create_player(user):
    ...
    new_player = {
        ...
        "party_id": None,
        "location": "تهران",
        "regional_assets": {},  # Format: {"region_name": {"soldiers": 0, "companies": []}}
        "is_alive": True
    }
...
def purchase_item(user_id, item_type, item_name, cost, region=None):
    """Purchase an item for a player in a specific region"""
    ...
    # Check if player is in a party and region is valid
    if not player.get('party_id'):
        return False, "برای خرید باید عضو حزبی باشید"
    
    party_regions = get_party_regions(player['party_id'])
    if region and region not in party_regions:
        return False, "منطقه انتخابی متعلق به حزب شما نیست"
    
    # Use current location if no region specified
    if not region:
        region = player.get('location', 'تهران')
    
    # Deduct coins
    new_coins = player['coins'] - cost
    
    if item_type == "soldier":
        # Add soldier to specific region
        regional_assets = get_player_regional_assets(user_id)
        if region not in regional_assets:
            regional_assets[region] = {"soldiers": 0, "companies": []}
        regional_assets[region]["soldiers"] += 1
        update_player(user_id, {'coins': new_coins, 'regional_assets': regional_assets})
    elif item_type == "company":
        # Add company to specific region
        regional_assets = get_player_regional_assets(user_id)
        if region not in regional_assets:
            regional_assets[region] = {"soldiers": 0, "companies": []}
        company_data = {"name": item_name, "type": "company", "daily_income": get_company_income(item_name)}
        regional_assets[region]["companies"].append(company_data)
        update_player(user_id, {'coins': new_coins, 'regional_assets': regional_assets})
    
    return True, f"خرید موفق! {item_name} در منطقه {region} خریداری شد."
...
def leave_party(user_id):
    ...
    update_player(user_id, {'party_id': None, 'regional_assets': {}})
...
def get_party_regions(party_id):
    """Get all regions controlled by a party"""
    party = get_party_by_id(party_id)
    if not party:
        return []
    return list(party.get('regions', {}).keys())

def get_player_regional_assets(user_id):
    """Get player's regional assets"""
    player = get_player(user_id)
    if not player:
        return {}
    return player.get('regional_assets', {})

def update_player_regional_assets(user_id, region, asset_type, value):
    """Update player's regional assets"""
    player = get_player(user_id)
    if not player:
        return False
    
    regional_assets = player.get('regional_assets', {})
    if region not in regional_assets:
        regional_assets[region] = {"soldiers": 0, "companies": []}
    
    if asset_type == "soldiers":
        regional_assets[region]["soldiers"] = value
    elif asset_type == "companies":
        regional_assets[region]["companies"] = value
    
    update_player(user_id, {'regional_assets': regional_assets})
    return True

def move_soldiers(user_id, from_region, to_region, soldier_count):
    """Move soldiers from one region to another"""
    player = get_player(user_id)
    if not player or not player.get('party_id'):
        return False, "شما عضو هیچ حزبی نیستید."
    
    party_id = player['party_id']
    party_regions = get_party_regions(party_id)
    
    if from_region not in party_regions or to_region not in party_regions:
        return False, "منطقه مورد نظر متعلق به حزب شما نیست."
    
    regional_assets = get_player_regional_assets(user_id)
    
    if from_region not in regional_assets:
        return False, "در منطقه مبدا سربازی ندارید."
    
    if regional_assets[from_region]["soldiers"] < soldier_count:
        return False, "سربازان کافی در منطقه مبدا موجود نیست."
    
    # Move soldiers
    regional_assets[from_region]["soldiers"] -= soldier_count
    if to_region not in regional_assets:
        regional_assets[to_region] = {"soldiers": 0, "companies": []}
    regional_assets[to_region]["soldiers"] += soldier_count
    
    update_player(user_id, {'regional_assets': regional_assets})
    return True, f"{soldier_count} سرباز از {from_region} به {to_region} منتقل شد."

def attack_region(attacker_id, attacker_region, target_party_id, target_region):
    """Attack another party's region"""
    attacker = get_player(attacker_id)
    if not attacker or not attacker.get('party_id'):
        return False, "شما عضو هیچ حزبی نیستید."
    
    attacker_party_id = attacker['party_id']
    attacker_party_regions = get_party_regions(attacker_party_id)
    target_party_regions = get_party_regions(target_party_id)
    
    if attacker_region not in attacker_party_regions:
        return False, "منطقه مهاجم متعلق به حزب شما نیست."
    
    if target_region not in target_party_regions:
        return False, "منطقه هدف متعلق به حزب مقصد نیست."
    
    # Get attacker's soldiers in the region
    attacker_assets = get_player_regional_assets(attacker_id)
    attacker_soldiers = attacker_assets.get(attacker_region, {}).get("soldiers", 0)
    
    if attacker_soldiers == 0:
        return False, "در منطقه مهاجم سربازی ندارید."
    
    # Get all defenders in target region
    target_party = get_party_by_id(target_party_id)
    target_members = target_party.get('members', [])
    
    total_defender_soldiers = 0
    defender_companies = []
    
    for member_id in target_members:
        member_assets = get_player_regional_assets(member_id)
        region_data = member_assets.get(target_region, {})
        total_defender_soldiers += region_data.get("soldiers", 0)
        defender_companies.extend(region_data.get("companies", []))
    
    # Simple combat calculation
    if attacker_soldiers > total_defender_soldiers:
        # Attacker wins
        return handle_combat_victory(attacker_id, attacker_region, target_party_id, target_region, defender_companies)
    else:
        # Defender wins
        return handle_combat_defeat(attacker_id, attacker_region)

def handle_combat_victory(attacker_id, attacker_region, target_party_id, target_region, defender_companies):
    """Handle combat victory - destroy companies and distribute money"""
    ...
    for company in defender_companies:
        total_company_value += company_values.get(company.get('name', ''), 0)
    
    # Get attacker's party members
    attacker_party = get_party_by_id(get_player(attacker_id)['party_id'])
    attacker_members = attacker_party.get('members', [])
    
    if attacker_members and total_company_value > 0:
        # Distribute money equally among attacking party members
        money_per_member = total_company_value // len(attacker_members)
        
        for member_id in attacker_members:
            member = get_player(member_id)
            if member:
                new_coins = member.get('coins', 0) + money_per_member
                update_player(member_id, {'coins': new_coins})
    
    # Remove all soldiers and companies from target region
    target_party = get_party_by_id(target_party_id)
    target_members = target_party.get('members', [])
    
    for member_id in target_members:
        member_assets = get_player_regional_assets(member_id)
        if target_region in member_assets:
            member_assets[target_region] = {"soldiers": 0, "companies": []}
            update_player(member_id, {'regional_assets': member_assets})
    
    return True, f"پیروزی! منطقه {target_region} تسخیر شد. {total_company_value:,} سکه بین اعضای حزب تقسیم شد."

def handle_combat_defeat(attacker_id, attacker_region):
    """Handle combat defeat - remove attacker's soldiers"""
    attacker_assets = get_player_regional_assets(attacker_id)
    if attacker_region in attacker_assets:
        attacker_assets[attacker_region]["soldiers"] = 0
        update_player(attacker_id, {'regional_assets': attacker_assets})
    
    return False, f"شکست! تمام سربازان شما در منطقه {attacker_region} از بین رفتند."
...
برای خرید اشتراک به آیدی @Nonobodynonono پیام بدید
    ...
    # Calculate total regional assets
    regional_assets = get_player_regional_assets(user_id)
    total_soldiers = sum(region_data.get("soldiers", 0) for region_data in regional_assets.values())
    total_companies = sum(len(region_data.get("companies", [])) for region_data in regional_assets.values())
...
💼 کل شرکت‌ها: {total_companies}
🗺️ مناطق فعال: {len(regional_assets)}
❤️ وضعیت: {'زنده' if player.get('is_alive', True) else 'مرده'}
    ...
    keyboard = [
        [
            InlineKeyboardButton("🗺️ مدیریت مناطق", callback_data="manage_regions"),
        ...
        [
            InlineKeyboardButton("🚛 جابجایی سرباز", callback_data="move_soldiers"),
...
یکی از شرکت‌ها را انتخاب کنید:
        ...
        player = get_player(user_id)
        if not player.get('party_id'):
            await query.edit_message_text("❌ برای خرید سرباز باید عضو حزبی باشید.")
            return
        
        # Show region selection for soldier purchase
        party_regions = get_party_regions(player['party_id'])
        if not party_regions:
            await query.edit_message_text("❌ حزب شما هیچ منطقه‌ای ندارد.")
            return
        
        keyboard = []
        for region in party_regions:
            keyboard.append([InlineKeyboardButton(f"🏛️ {region}", callback_data=f"buy_soldier_{soldier_type}_{region}")])
        
        keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data="buy_soldiers")])
        markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(
            f"⚔️ خرید {name}\n💰 قیمت: {cost:,} سکه\n\n🗺️ منطقه مورد نظر را انتخاب کنید:",
            reply_markup=markup
        )
    
    elif data.startswith("buy_soldier_") and "_" in data and len(data.split("_")) >= 4:
        # Handle soldier purchase with specific region
        parts = data.split("_")
        if len(parts) >= 4:
            soldier_type = parts[2]
            region = "_".join(parts[3:])  # Handle regions with underscores
            ...
            success, message = purchase_item(user_id, "soldier", name, cost, region)
            
            if success:
                player = get_player(user_id)
                regional_assets = get_player_regional_assets(user_id)
                region_soldiers = regional_assets.get(region, {}).get("soldiers", 0)
                
                result_message = f"""
✅ {message}

💰 موجودی جدید: {player.get('coins', 0):,} سکه
⚔️ سربازان در {region}: {region_soldiers}
                """
            else:
                result_message = f"❌ {message}"
            
            keyboard = [
                [InlineKeyboardButton("🔙 بازگشت به فروشگاه", callback_data="show_shop")],
                [InlineKeyboardButton("🛒 خرید بیشتر", callback_data="buy_soldiers")]
            ]
            markup = InlineKeyboardMarkup(keyboard)
            
            await query.edit_message_text(result_message.strip(), reply_markup=markup)
        ...
        player = get_player(user_id)
        if not player.get('party_id'):
            await query.edit_message_text("❌ برای خرید شرکت باید عضو حزبی باشید.")
            return
        
        # Show region selection for company purchase
        party_regions = get_party_regions(player['party_id'])
        if not party_regions:
            await query.edit_message_text("❌ حزب شما هیچ منطقه‌ای ندارد.")
            return
        ...
        keyboard = []
        for region in party_regions:
            keyboard.append([InlineKeyboardButton(f"🏛️ {region}", callback_data=f"buy_company_{company_type}_{region}")])
        
        keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data="buy_company")])
        markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(
            f"🏭 خرید {name}\n💰 قیمت: {cost:,} سکه\n💵 درآمد روزانه: {get_company_income(name)} سکه\n\n🗺️ منطقه مورد نظر را انتخاب کنید:",
            reply_markup=markup
        )
    
    elif data.startswith("buy_company_") and "_" in data and len(data.split("_")) >= 4:
        # Handle company purchase with specific region
        parts = data.split("_")
        if len(parts) >= 4:
            ...
            success, message = purchase_item(user_id, "company", name, cost, region)
            
            if success:
                player = get_player(user_id)
                regional_assets = get_player_regional_assets(user_id)
                region_companies = len(regional_assets.get(region, {}).get("companies", []))
...
💰 موجودی جدید: {player.get('coins', 0):,} سکه
🏭 شرکت‌ها در {region}: {region_companies}
💵 درآمد روزانه: {daily_income} سکه
            ...
            keyboard = [
                [InlineKeyboardButton("🔙 بازگشت به فروشگاه", callback_data="show_shop")],
                [InlineKeyboardButton("🛒 خرید بیشتر", callback_data="buy_company")]
            ]
            markup = InlineKeyboardMarkup(keyboard)
            
            await query.edit_message_text(result_message.strip(), reply_markup=markup)
    
    elif data == "my_investments" or data == "manage_investments":
        player = get_player(user_id)
        regional_assets = get_player_regional_assets(user_id)
        
        if not regional_assets:
            message = "💼 شما هیچ سرمایه‌گذری ندارید.\n\nبرای شروع، از فروشگاه شرکت خریداری کنید."
        else:
            total_income = 0
            message = f"💼 سرمایه‌گذری‌های شما:\n\n"
            
            for region, assets in regional_assets.items():
                ...
                if companies:
                    ...
                    for company in companies:
                        ...
                        message += f"   • {company['name']} - درآمد: {daily_income} سکه/روز\n"
                    message += "\n"
            
            message += f"💵 کل درآمد روزانه: {total_income} سکه"
        
        keyboard = [[InlineKeyboardButton("🔙 بازگشت", callback_data="show_profile")]]
        markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(message, reply_markup=markup)
    
    elif data == "manage_regions":
        # Show regional management interface
        player = get_player(user_id)
        if not player.get('party_id'):
            await query.edit_message_text("❌ برای مدیریت مناطق باید عضو حزبی باشید.")
            return
        
        party_regions = get_party_regions(player['party_id'])
        regional_assets = get_player_regional_assets(user_id)
        
        message = "🗺️ مدیریت مناطق حزب شما:\n\n"
        
        for region in party_regions:
            soldiers = regional_assets.get(region, {}).get("soldiers", 0)
            companies = len(regional_assets.get(region, {}).get("companies", []))
            message += f"🏛️ {region}:\n"
            message += f"   ⚔️ سربازان: {soldiers}\n"
            message += f"   🏭 شرکت‌ها: {companies}\n\n"
        
        keyboard = [
            [InlineKeyboardButton("🚛 جابجایی سرباز", callback_data="move_soldiers")],
            [InlineKeyboardButton("⚔️ حمله به دشمن", callback_data="attack_menu")],
    ...
    elif data == "move_soldiers":
        # Show soldier movement interface
        player = get_player(user_id)
        if not player.get('party_id'):
            await query.edit_message_text("❌ برای جابجایی سرباز باید عضو حزبی باشید.")
            return
        
        party_regions = get_party_regions(player['party_id'])
        regional_assets = get_player_regional_assets(user_id)
        
        # Show regions with soldiers
        keyboard = []
        for region in party_regions:
            soldiers = regional_assets.get(region, {}).get("soldiers", 0)
            if soldiers > 0:
                keyboard.append([InlineKeyboardButton(f"🏛️ {region} ({soldiers} سرباز)", callback_data=f"move_from_{region}")])
        
        if not keyboard:
            await query.edit_message_text("❌ شما در هیچ منطقه‌ای سربازی ندارید.")
            return
        
        keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data="manage_regions")])
        markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text("🚛 منطقه مبدا را انتخاب کنید:", reply_markup=markup)
    
    elif data == "attack_menu":
        # Show attack interface
        player = get_player(user_id)
        if not player.get('party_id'):
            await query.edit_message_text("❌ برای حمله باید عضو حزبی باشید.")
            return
        
        party_regions = get_party_regions(player['party_id'])
        regional_assets = get_player_regional_assets(user_id)
        
        # Show regions with soldiers for attack
        keyboard = []
        for region in party_regions:
            soldiers = regional_assets.get(region, {}).get("soldiers", 0)
            if soldiers > 0:
                keyboard.append([InlineKeyboardButton(f"⚔️ از {region} ({soldiers} سرباز)", callback_data=f"attack_from_{region}")])
        ...
        await query.edit_message_text("⚔️ منطقه مهاجم را انتخاب کنید:", reply_markup=markup)
    
    elif data.startswith("move_from_"):
        # Handle soldier movement - show destination selection
        from_region = data.replace("move_from_", "")
        player = get_player(user_id)
        party_regions = get_party_regions(player['party_id'])
        
        # Show available destinations (other regions in same party)
        keyboard = []
        for region in party_regions:
            if region != from_region:
                keyboard.append([InlineKeyboardButton(f"🏛️ {region}", callback_data=f"move_{from_region}_to_{region}")])
        
        keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data="move_soldiers")])
        markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(f"🚛 انتقال سربازان از {from_region}\n\n📍 منطقه مقصد را انتخاب کنید:", reply_markup=markup)
    
    elif data.startswith("move_") and "_to_" in data:
        # Handle actual soldier movement
        parts = data.split("_to_")
        if len(parts) == 2:
            from_region = parts[0].replace("move_", "")
            to_region = parts[1]
            
            player = get_player(user_id)
            regional_assets = get_player_regional_assets(user_id)
            available_soldiers = regional_assets.get(from_region, {}).get("soldiers", 0)
            
            if available_soldiers == 0:
                await query.edit_message_text("❌ در این منطقه سربازی ندارید.")
                return
            
            # For simplicity, move all soldiers (can be enhanced later for partial moves)
            success, message = move_soldiers(user_id, from_region, to_region, available_soldiers)
    ...
    elif data.startswith("attack_from_"):
        # Handle attack - show enemy party selection
        from_region = data.replace("attack_from_", "")
        player = get_player(user_id)
        attacker_party_id = player['party_id']
        
        # Show enemy parties
        all_parties = load_json(PARTIES_FILE)
        enemy_parties = [p for p in all_parties if p['id'] != attacker_party_id]
        
        keyboard = []
        for party in enemy_parties:
            keyboard.append([InlineKeyboardButton(f"⚔️ {party['name']}", callback_data=f"attack_{from_region}_target_{party['id']}")])
        
        keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data="attack_menu")])
        markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(f"⚔️ حمله از {from_region}\n\n🎯 حزب دشمن را انتخاب کنید:", reply_markup=markup)
    
    elif data.startswith("attack_") and "_target_" in data:
        # Handle attack - show enemy regions
        parts = data.split("_target_")
        if len(parts) == 2:
            from_region = parts[0].replace("attack_", "")
            target_party_id = int(parts[1])
            
            target_party = get_party_by_id(target_party_id)
            if not target_party:
                await query.edit_message_text("❌ حزب مقصد یافت نشد.")
                return
            
            target_regions = get_party_regions(target_party_id)
            
            keyboard = []
            for region in target_regions:
                keyboard.append([InlineKeyboardButton(f"🏛️ {region}", callback_data=f"attack_{from_region}_party_{target_party_id}_region_{region}")])
            
            keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data=f"attack_from_{from_region}")])
            markup = InlineKeyboardMarkup(keyboard)
            
            await query.edit_message_text(f"⚔️ حمله از {from_region} به {target_party['name']}\n\n🎯 منطقه مقصد را انتخاب کنید:", reply_markup=markup)
    
    elif data.startswith("attack_") and "_party_" in data and "_region_" in data:
        # Handle actual attack
        parts = data.split("_party_")
        if len(parts) == 2:
            from_region = parts[0].replace("attack_", "")
            party_region_part = parts[1].split("_region_")
            if len(party_region_part) == 2:
                target_party_id = int(party_region_part[0])
                target_region = party_region_part[1]
                
                success, message = attack_region(user_id, from_region, target_party_id, target_region)
                
                if success:
                    keyboard = [
                        [InlineKeyboardButton("🗺️ مدیریت مناطق", callback_data="manage_regions")],
                        [InlineKeyboardButton("⚔️ حمله مجدد", callback_data="attack_menu")],
                        [InlineKeyboardButton("🔙 بازگشت", callback_data="show_profile")]
                    ]
                    markup = InlineKeyboardMarkup(keyboard)
                    await query.edit_message_text(f"🎉 {message}", reply_markup=markup)
        ...
        initial_parties = [
            {
                ...
                "members": [], 
                "regions": {
                    "تهران": {"soldiers": 0, "companies": []},
                    "همدان": {"soldiers": 0, "companies": []},
                    "فارس": {"soldiers": 0, "companies": []},
                    "خوزستان": {"soldiers": 0, "companies": []},
                    "خراسان": {"soldiers": 0, "companies": []},
                    "بلوچستان": {"soldiers": 0, "companies": []}
                }
            },
            {
                "id": 2, 
                ...
                "members": [], 
                "regions": {
                    "مازندران": {"soldiers": 0, "companies": []},
                    "آذربایجان": {"soldiers": 0, "companies": []},
                    "کردستان": {"soldiers": 0, "companies": []},
                    "لرستان": {"soldiers": 0, "companies": []},
                    "اصفهان": {"soldiers": 0, "companies": []},
                    "گیلان": {"soldiers": 0, "companies": []}
                }
            },
            {
                "id": 3, 
                ...
                "members": [], 
                "regions": {
                    "یزد": {"soldiers": 0, "companies": []},
                    "کرمان": {"soldiers": 0, "companies": []},
                    "هرمزگان": {"soldiers": 0, "companies": []},
                    "بوشهر": {"soldiers": 0, "companies": []},
                    "کهگیلویه": {"soldiers": 0, "companies": []},
                    "چهارمحال": {"soldiers": 0, "companies": []}
                }
            },
            {
                "id": 4, 
                ...
                "members": [], 
                "regions": {
                    "سمنان": {"soldiers": 0, "companies": []},
                    "قزوین": {"soldiers": 0, "companies": []},
                    "قم": {"soldiers": 0, "companies": []},
                    "البرز": {"soldiers": 0, "companies": []},
                    "مرکزی": {"soldiers": 0, "companies": []},
                    "زنجان": {"soldiers": 0, "companies": []}
                }
            },
            {
                "id": 5, 
                ...
                "members": [], 
                "regions": {
                    "گلستان": {"soldiers": 0, "companies": []},
                    "اردبیل": {"soldiers": 0, "companies": []},
                    "ایلام": {"soldiers": 0, "companies": []},
                    "کرمانشاه": {"soldiers": 0, "companies": []},
                    "خراسان شمالی": {"soldiers": 0, "companies": []},
                    "خراسان جنوبی": {"soldiers": 0, "companies": []}
                }
            },
            {
                "id": 6, 
                ...
                "members": [], 
                "regions": {
                    "خراسان رضوی": {"soldiers": 0, "companies": []},
                    "فارس مرکزی": {"soldiers": 0, "companies": []},
                    "اصفهان شرقی": {"soldiers": 0, "companies": []},
                    "تهران شرقی": {"soldiers": 0, "companies": []},
                    "خوزستان شرقی": {"soldiers": 0, "companies": []},
                    "بلوچستان شرقی": {"soldiers": 0, "companies": []}
                }
            }
        ]
